...
def process_case(args):
    with warnings.catch_warnings():
        ...
        try:
            ...
            eeg = []
            for D_idx in range(0,len(eeg_data)):
                Ds = eeg_data[D_idx]
            ...
            for tau in ["", "_tau10"]:
                for emb in [30, 50, 64]:
                    ...
                    np.save(f'{san_disk}/eegs/method_5/{band}/{speed}/bb{bb}_ut{ut}_cc_{typp}_emb{emb}{tau}.npy', dists_2)


            return bb, ut, band, speed, typp
...
if __name__ == "__main__":
    ...
    tasks = [(m, n, speed, band, typp) 
             for m in range(1,7)
             for n in range(len(auds))
             for speed in ["slow", "fast"]
             for band in ["theta", "alpha", "beta"]
             for typp in ["sphere", "square"]]
