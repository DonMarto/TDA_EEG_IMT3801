...
# df_scores.to_csv("scores_7.csv")

# df_scores = pd.DataFrame(columns=["Model", "Method", "Subject", "Trial", "Speed", "Band", "Emb", "Tau", "Type", "Score"])
# df_scores.to_csv("scores_8.csv")
...
def regress_models_2(X_raw, y_raw, subjects, method = "Ridge", candidates = [{"alpha": 1}, {"alpha": 2}], verb=0):
    ...
    for X, y in zip(X_raw, y_raw):
        ...
        yr = y[:target_len]
        y_flat = yr.reshape(-1)
        y_flat = y_flat[::59]  # truncate or pad as needed
...
def process_case(args):
    with warnings.catch_warnings():
        ...
        try:
            ...
            df_scores = pd.read_csv("scores_7.csv")
            df_scores = pd.concat([df_scores, pd.DataFrame(rows)], ignore_index=True)
            df_scores.to_csv("scores_7.csv", index=False)
            ...
            # df_scores = pd.read_csv("scores_8.csv")
            # df_scores = pd.concat([df_scores, pd.DataFrame(rows)], ignore_index=True)
            # df_scores.to_csv("scores_8.csv", index=False)
...
if __name__ == "__main__":
    ...
    tasks = [(0,0,1,1), (2,0,1,0), (0,0,2,0)]#, typp in enumerate(["sphere", "square"]):]
